...
<h3>Liste des sujets de formation</h3>
     <table id="sujetsTable" border="1" class="table table-striped table-responsive">
           ...
           <c:if test="${jsonSujets.length() > 0}"> 
	           <c:forEach begin="0" end="${jsonSujets.length() -1}" var="index">
	            <tr>
	                <td>${jsonSujets.getJSONObject(index).getString("code")}</td>
	                <td>${jsonSujets.getJSONObject(index).getString("libelle")}</td>
	                <td>${jsonSujets.getJSONObject(index).getString("formateur")}</td>
	                <td>${jsonSujets.getJSONObject(index).getString("prerequis")}</td>
	                <td></td>
	            </tr>
	        </c:forEach>
        </c:if>
